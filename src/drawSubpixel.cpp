...
void
SubpixelWorkspace::drawLine(double rowStart, double colStart, double rowEnd, double colEnd, const cv::Scalar &color) {
...
void
SubpixelWorkspace::drawRect(const std::vector<cv::Point2d> &corners, const cv::Scalar &color, bool drawCorners) {
    double rowStart;
    double colStart;
    double rowEnd;
    double colEnd;
...
void SubpixelWorkspace::drawEllipse(double row, double col, double phi, double shortRadius, double longRadius,
    ...
    ellipse(zoomedImage, Point2d(zoomedCol, zoomedRow), Size(zoomedShortRad, zoomedLongRad), (-phi) * 180 / CV_PI, 0, 360,
            color);
}
